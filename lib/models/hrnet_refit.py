...
class PoseHighResolutionNet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.downsample_stage_1 = self._make_downsample_layer(1, pre_stage_channels[0])
        self.downsample_stage_2 = self._make_downsample_layer(1, pre_stage_channels[1])
        self.downsample_stage_3 = self._make_downsample_layer(1, pre_stage_channels[2])
        self.downsamples = nn.ModuleList([self.downsample_stage_1,
                                          self.downsample_stage_2,
                                          self.downsample_stage_3])

        self.final_layer = nn.Sequential(
                nn.Conv2d(in_channels=pre_stage_channels[-1],
                          out_channels=pre_stage_channels[-1],
                          kernel_size=1, stride=1, padding=0, 
                          bias=False),
                nn.BatchNorm2d(pre_stage_channels[-1], momentum=BN_MOMENTUM),
    ...
    def _make_downsample_layer(self, num_layers, num_channel, kernel_size=3):
        ...
        for i in range(num_layers):
            layers.append(
                nn.Conv2d(
                    in_channels=num_channel, out_channels=num_channel*2,
                    kernel_size=kernel_size, stride=2, padding=1, bias=False,
                )
            )
            layers.append(nn.BatchNorm2d(num_channel*2, momentum=BN_MOMENTUM))
            layers.append(nn.ReLU(inplace=True))
    ...
    def forward(self, x):
        ...
        feature_map = torch.cat([x[0], x1, x2, x3], 1)


        # Downsample to get feature vector
        y = x[0]
        for i in range(len(self.downsamples)):
            y = self.downsamples[i](y) + x[i+1]

        y = self.final_layer(y)
        
        #y = torch.mean(y, [-1, -2])
        y = F.avg_pool2d(y, kernel_size=y.size(-1))
        y = y.view(y.size(0), -1)
    ...
    def init_weights(self, pretrained='', device='cpu'):
        ...
        if os.path.isfile(pretrained):
            ...
            print('Loaded pretrained HRnet from', pretrained)

        elif pretrained:
            pass
            print('Pretrained hrnet model not available!!')
...
def hrnet_w32(
    ...
    if crop_size == 224 or crop_size == 256:
        pretrained_ckpt = 'data/pretrain/pose_hrnet_w32_256x192.pth'
    elif crop_size == 384:
        pretrained_ckpt = 'data/pretrain/pose_hrnet_w32_384x288.pth'
    elif crop_size == 512:
        pretrained_ckpt = 'data/pretrain/pose_dekr_hrnetw32_coco.pth'
...
def hrnet_w48(
    ...
    if crop_size == 224 or crop_size == 256:
        pretrained_ckpt='data/pretrain/pose_hrnet_w48_256x192.pth'
    elif crop_size == 384 or crop_size == 512:
        pretrained_ckpt = 'data/pretrain/pose_hrnet_w48_384x288.pth'
...
def hrnet_w32_cls(
    ...
    pretrained_ckpt = 'data/pretrain/hrnetv2_w32_imagenet_pretrained.pth'
...
def hrnet_w48_cls(
    ...
    pretrained_ckpt='data/pretrain/hrnetv2_w48_imagenet_pretrained.pth'
    # pretrained_ckpt='data/pretrain/HRNet_W48_C_cosinelr_cutmix_300epoch.pth.tar'
